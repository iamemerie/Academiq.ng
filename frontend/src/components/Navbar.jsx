...
import { Link, useLocation } from "react-router-dom"; // Using useLocation for active link styling

function Navbar() {
  const fullName = localStorage.getItem("fullName");
  const role = localStorage.getItem("role");
  const location = useLocation(); // Tracks what page the user is currently on
  ...
  const getLinkClass = (path) => {
    const baseClass = "text-sm font-medium transition duration-150 ";
